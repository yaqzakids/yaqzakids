      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 py-12">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {LEARNING_PATHS_HOME.map((p) => (
            <div key={p.name} className={`bg-white rounded-2xl shadow-sm border-t-4 ${p.border} p-6`}>
              <p className="text-3xl mb-2">{p.emoji}</p>
              <h2 className="font-bold text-navy mb-1">{p.name}</h2>
              <p className="text-xs text-muted mb-4">{p.articles} stories · Ages 9–12</p>
              <TealProgressBar value={0} className="mb-4 opacity-40" />
              <Link
                to="/signup"
